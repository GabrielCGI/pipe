...
def main(log_file):
    ...
    plt.axhline(y=threshold.total_seconds(), linestyle='--', label='Bottleneck Threshold')
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        # If user provided a path from the command line
        log_file = sys.argv[1]
    else:
        # Fallback to some default file path if no args provided
