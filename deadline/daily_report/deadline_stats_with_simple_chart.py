...
# -*- coding: utf-8 -*-
"""
Script pour extraire les statistiques Deadline et générer un rapport HTML avec graphiques
"""
...
class DeadlineStatsParser:
    ...
    def convert_time_to_minutes(self, time_str: str) -> float:
        """Convertit une durée HH:MM:SS en minutes"""
        try:
    ...
    def generate_html_report(self, jobs: List[Dict], worker_stats: Dict, output_dir: str = r"R:\devMathieu\deadlineStats", output_filename: str = None):
        """
        Génère un rapport HTML avec les statistiques et un graphique simple
        ...
        for job in jobs:
            user = job['user']
            time_minutes = self.convert_time_to_minutes(job['avg_frame_render_time'])
            if time_minutes > 0:
                user_times[user].append(time_minutes)
        ...
        chart_data.sort(key=lambda x: x['avg_time'], reverse=True)
        
        # Préparer les données pour Chart.js
        chart_labels = [d['user'] for d in chart_data]
        chart_values = [d['avg_time'] for d in chart_data]
...
<body>
    <div class="container">
        ...
        <div class="section">
            <h2>📈 Temps de Rendu Moyen par Utilisateur</h2>
    ...
    <script>
        ...
        if (chartLabels.length > 0) {{
            ...
            const myChart = new Chart(ctx, {{
                ...
                data: {{
                    labels: chartLabels,
                    datasets: [{{
                        label: 'Temps moyen de rendu (minutes)',
                        data: chartValues,
            ...
            document.querySelector('.chart-container').innerHTML = '<div class="no-data">Aucune donnée de temps de rendu disponible</div>';
        }}
        ...
        print(f"\n✅ Rapport HTML généré avec graphique:")
        ...
        if chart_data:
            ...
            for d in chart_data[:5]:  # Afficher les 5 premiers
                print(f"   • {d['user']}: {d['avg_time']:.2f} min (moyenne sur {d['job_count']} jobs)")
            if len(chart_data) > 5:
                print(f"   ... et {len(chart_data) - 5} autres utilisateurs")
