...
def clean_string(text):
    ...
    """
    return _INVALID_NAME_CHARS.sub("_", text)


def get_network_editor():
...
def get_materials(node):
    ...
    if not stage:
        hou.ui.displayMessage(
            "Failed to get USD stage from node.", severity=hou.severityType.Error
        )
...
def create_karma_materials(materials_dict, material_library):
    ...
    """
    network_editor = get_network_editor()
    ...
    for i, (mtl_path, child_list) in enumerate(materials_dict.items()):
        ...
        if short_mtl_name not in existing_children:
            ...
            kma_subnet = create_karma_mat_builder(
                material_library, short_mtl_name, standard_surface_name
            )
            ...
            print_info(
                f"Material name {short_mtl_name} already exists in {material_library}, skipping"
            )
