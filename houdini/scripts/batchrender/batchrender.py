...
UI_RESSOURCES_DIR = os.path.join(os.path.dirname(__file__), "ressources")
...
class ShotItem(QtWidgets.QWidget):
    def __init__(
        self, text, item: QtWidgets.QTreeWidgetItem, batch_selector, parent=None
    ):
        ...
        btn_check_all.clicked.connect(lambda: self.set_check_all(True))
        lo_item.addWidget(btn_check_all)

        btn_uncheck_all = QtWidgets.QPushButton("Uncheck All")
        btn_uncheck_all.clicked.connect(lambda: self.set_check_all(False))
...
class BatchSelector(QtWidgets.QDialog):
    def __init__(
        self, render_nodes: list[hou.LopNode], parent: QtWidgets.QWidget = None
    ):
    ...
    def setup_UI(self):
        ...
        select_all_button.clicked.connect(lambda: self.set_check_all(True))
        lo_selection.addWidget(select_all_button)

        deselect_all_button = QtWidgets.QPushButton(text="Uncheck All")
        deselect_all_button.clicked.connect(lambda: self.set_check_all(False))
    ...
    def create_render_tree(self):
        ...
        for node in self.render_nodes:
            ...
            for id_exec in range(1, nb_exec + 1):
                ...
                data["executions"].append({"shot": exec_shot, "layer": exec_layer})
                ...
                exec_item.setFlags(
                    exec_item.flags() & ~QtCore.Qt.ItemFlag.ItemIsUserCheckable
                )
    ...
    def submit(self):
        ...
        for id_shot in range(root.childCount()):
            ...
            for id_exec in range(shot.childCount()):
                ...
                parm_name = f"execution_enabled_{execution_data}"
        ...
        self.submitter = farmsubmitter.Farm_Submitter(LOP_RENDER, states, kwargs)
    ...
    def parse_execution(self, id_exec, node):
        ...
        edit_context_option = [
            n for n in node.inputs() if n.type().name() == "editcontextoptions"
        ]
...
def main():
    ...
    render_nodes = list(
        hou.lopNodeTypeCategory().nodeType("prism::LOP_Render::1.0").instances()
    )
