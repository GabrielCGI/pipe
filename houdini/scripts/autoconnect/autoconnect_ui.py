...
class MapWidget(QtWidgets.QWidget):
    ...
    def __init__(
            self, mapName: str,
            current_map: str=DEFAULT_VERSION_VALUE, new_map: str=DEFAULT_VERSION_VALUE,
            listVersion: list[str]=[DEFAULT_VERSION_VALUE], versioning=False,
            parent=None):
        """
...
class ShaderWidget(QtWidgets.QTreeWidget):
    ...
    def __init__(
            self, shaderName: str, shader: sh.Shader,
            maps: list[MapWidget], versioning: bool=False,
            color: hou.Color=None, parent=None):
        """
    ...
    def initializeShaderWidgets(self):
        ...
        for shader in self.shaders:
            ...
            for map in shader.unversionned_map:
                ...
                mapWidget = MapWidget(
                    mapName=map.name, current_map=current_map.name,
                    new_map=map.name, listVersion=[map.name],
                    versioning=False)
            ...
            for map in shader.latest_maps:
                ...
                mapWidget = MapWidget(
                    mapName=map.name, current_map=old_version,
                    new_map=map.version, listVersion=listVersion,
                    versioning=True)
                listMapWidgets.append(mapWidget)
            versioning = len(shader.latest_maps)
            shaderWidget = ShaderWidget(
                shaderName=shader.name, shader=shader, maps=listMapWidgets,
                versioning=versioning, color=subnet_color
            )
    ...
    def parseMatsBySubnets(self, subnets: list[hou.VopNode]):
        ...
        for subnet in maps_by_subnets:
            ...
            for map_path in maps_by_subnets.get(subnet, []):
                ...
                globpath = (
                    map_path[:versionSpan[0]]
                    + "*"
                    + map_path[versionSpan[1]:]
                )
    ...
    def refreshFromFiles(self, maps_by_subnets: dict):
        ...
        if len(self.selectedShaders) and self.shaders:
            ...
            for shader in self.selectedShaders:
                if not shader in maps_by_subnets.keys():
