...
def parse_and_cache(file: str, shaders: list[sh.Shader]) -> list[sh.Shader]:
    ...
    shader_name = get_shader_name(file_name)
...
def get_shader(shader_name: str, filepaths: list[str]) -> sh.Shader:
    """Create and return a shader with shader_name from maps in filepaths.
...
def get_existing_subnet(material_library: hou.Node, shader_name: str):
    ...
    for subnet in material_library_subnetworks:
        if shader_name in subnet.name().lower():
            return subnet
    return None
