...
# Optical Flares Configuration
    offset_path = "R:/pipeline/networkInstall/Nuke/nuke15+_configs/OpticalFlares"
    optical_flares_paths = {
        ("13.1", True): os.path.join(offset_path, "OpticalFlares_Nuke_13.1_Node-Locked_1.0.9"),
        ("13.1", False): os.path.join(offset_path, "OF_Render_13.1"),
        ("15.1", True): os.path.join(offset_path, "OpticalFlares_Nuke_15.1_Node-Locked_1.0.94"),
        ("15.1", False): os.path.join(offset_path, "OF_Render_15.1"),
    }
    is_gui = nuke.GUI
    plugin_path = optical_flares_paths.get((nuke_version, is_gui))
    if plugin_path:
        nuke.pluginAddPath(os.path.join(plugin_path, "plugin/Windows"))

    os.environ["OPTICAL_FLARES_LICENSE_PATH"] = (
    r"C:\Program Files\Nuke13.1v5" if (nuke_version == "13.1" and is_gui) else
    r"C:\Program Files\Nuke15.1v5" if (nuke_version == "15.1" and is_gui) else
    r"R:\pipeline\networkInstall\Nuke\nuke15+_configs\OpticalFlares\licenses"
)
    os.environ["OPTICAL_FLARES_PRESET_PATH"] = os.path.join(plugin_path, "Textures-And-Presets")
