...
class MayaBatchPlugin(DeadlinePlugin):
    ...
    def StartJob( self ):
        ...
        #HACK BLOOM TO DISABLE PATH MAPPING ON RANCH
        job = self.GetJob()
        my_comment = job.Comment
        if my_comment == "ranch_path_mapping":

            self.LogInfo( "RANCH PATH MAPPING: ON" )
        else:
            self.EnablePathMapping = False
            self.LogInfo( "RANCH PATH MAPPING: OFF (job comment is not ranch_path_mapping" )
        #END HACK BLOOM
        ...
        #HACK BLOOM TO DISABLE PATH MAPPING ON RANCH (ajout d'une condiftion pour remapper le sceneFileName)
        if my_comment == "ranch_path_mapping":
    ...
    def RenderTasks( self ):
        ...
        #BLOOM EDIT ! REMOVE PATH MAPPING ON RENDER OUTPUT !
                ...
                if self.FirstTask:
                    ...
                    if self.XGenPathMapping:
                        ...
                        if len( mappings ) > 0:
                            ...
                            scriptBuilder.AppendLine( '}' )
                            scriptBuilder.AppendLine()
            #BLOOM EDIT - DISABLE PATH MAPPING FOR ASS. IT WAS PRODUCING ERROR WITH ASS SEQUENCES. FIRST FILE WAS COPIED BUT THEN THE OTHER WENT MISSING
