...
def run():
    # APPLY ENV VARIABLE FOR PATH MAMMINF IF IT's A RANCH
    if os.environ['COMPUTERNAME'].startswith('RANCH'):
        mel.eval('print "+-- Illogic path mapping because computer is RANCH\\n";')
        mel.eval('print "| Repath all Reference paths to RANCH paths\\n";')
        __repath_references()
        mel.eval('print "| Set all path of Files, Images and StandIns to relative\\n";')
        __repath_files_to_relative()
    else:
