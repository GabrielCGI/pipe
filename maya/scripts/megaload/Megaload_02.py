...
def retrieve_maps_3dmodel(asset):
	...
	matching_files["Color"] = 			scan_map("_Albedo", asset.path)
	matching_files["SpecRoughness"] = 	scan_map("_Roughness",asset.path)
	matching_files["Metalness"] = 		scan_map("_Metalness",asset.path)
	matching_files["Normal"] = 			scan_map("_Normal",asset.path)
	matching_files["Diplace"] = 		scan_map("_Displacement",asset.path)
...
def load_object(asset,shading_group):
	#get objs in scene before import
	objs_before = set(pm.ls(dag=True, long=True))

	# .fbx files listing
	fbx_files = [f for f in os.listdir(asset.path) if f.startswith(asset.name) and f.endswith('.fbx')]
	if fbx_files:

		# Import first matching file
		pm.importFile(os.path.join(asset.path, fbx_files[0]))
		# Get imported objs
...
def run(directory):
	...
	if asset.type == "3dmodel" or asset.type == "3dplant":
		load_object(asset,shading_group)
