...
STYLESHEET = ""
URL = os.environ.get("RLM_URL", "")
PORT = os.environ.get("RLM_PORT", "")
...
class NukeLicenseParser(Qt.QDialog):
    def __init__(self, parent=None):
        ...
        try:
            ...
            logger.warning(f"Could not load stylesheet:\n\t{e}")
    ...
    def create_list_license(self):
        ...
        for i, pool in enumerate(self.data):
            data: dict = self.data[pool]

            license_pool = Qt.QTableWidgetItem(pool)
            license_usage = Qt.QTableWidgetItem(
                "X" if not (data.get("limited")) else ""
            )
            license_limited = Qt.QTableWidgetItem("X" if data.get("limited") else "")
            license_user = Qt.QTableWidgetItem(
                data.get("user") if data.get("user") else ""
            )
            ...
            if data.get("used"):
                ...
                license_remove.clicked.connect(
                    lambda checked, p=pool: self.confirmation_dialog(p)
                )
        ...
        self.listLicenses.horizontalHeader().setSectionResizeMode(
            Qt.QHeaderView.ResizeMode.Stretch
        )

        self.mainLayout.addWidget(self.listLicenses)

    def confirmation_dialog(self, pool: dict):
        ...
        self.confirmDialog.setText(
            "Are you sure you want to remove " + current_user + "'s session?"
        )
        ...
        self.confirmDialog.okButton.clicked.connect(
            lambda: self.remove_license_parallel(pool)
        )
        self.confirmDialog.exec()

    def remove_license_parallel(self, pool: dict):
        """
        ...
        max_workers = min(
            12, len(handles)
        )  # Gotta adapt this to see how far we can push it
        ...
        logger.info(f"{socket.gethostname()} has remove {user} license")
    ...
    def generate_summary(self):
        ...
        summary = (
            f"Nuke15 licenses used : {self.usedNuke15}/{self.totalNuke15} \n"
            f"Nuke13 licenses used : {self.usedNuke13}/{self.totalNuke13} \n"
            f"Total licenses used : {self.usedLicenses}/{self.totalLicenses}"
        )
        logger.info(summary)
