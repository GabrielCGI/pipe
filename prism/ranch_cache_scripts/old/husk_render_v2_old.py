...
# Setup environment
os.environ["HOUDINI_PACKAGE_DIR"] = "C:/tmp/houdinipackage/" #GABIIIIIIIII
os.environ["PXR_AR_DEFAULT_SEARCH_PATH"] = (f"{LOCAL_DRIVE_I};{LOCAL_DRIVE_R}") #GABIIIIIIIII
env_copy = dict(os.environ) #GABIIIIIIIII

print("HOUDINI_PACKAGE_DIR:", os.environ["HOUDINI_PACKAGE_DIR"]) #GABIIIIIIIII
print("ENV keys:", list(env_copy.keys())) #GABIIIIIIIII

computer_name = os.getenv('COMPUTERNAME', '') #GABIIIIIIIII
if computer_name.startswith("RANCH") and "nocache" not in usd_file_path: #GABIIIIIIIII 
    usd_file_path, img_output, env_copy = remap_ranch_usd_and_output(usd_file_path, img_output, env_copy) #GABIIIIIIIII
else:
    print(f"No ranch path mapping for {computer_name}") #GABIIIIIIIII
...
args = [
    ...
    "--exrmode", "1", #GABIIIIIIIII
    "--autocrop", "C,A", #GABIIIIIIIII
]
...
proc = subprocess.Popen(args, stdout=subprocess.PIPE, env=env_copy) #GABIIIIIIIII
read_stdout(proc)
...
if proc.returncode:
    ...
    if computer_name.startswith("RANCH"): #GABIIIIIIIII
        print("Start copying EXRs back to network...") #GABIIIIIIIII
        shot_dir = os.path.dirname(img_output) #GABIIIIIIIII
        copy_exr_from_ranch_to_network(
            ranch_shot_exr_dir=shot_dir, #GABIIIIIIIII
            ranch_out_exr_dir=RANCH_OUT_EXR_DIR,
            is_test_mode=IS_EXR_COPY_ONLY_TEST_MODE, #GABIIIIIIIII
            delete_local_exr_after_copy = DELETE_LOCAL_EXR_AFTER_COPY # Set False to do real copy & delete
        )
    else:
        print("Skipping EXR copy-back") #GABIIIIIIIII
