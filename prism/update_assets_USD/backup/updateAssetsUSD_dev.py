...
class mainInterface(Qt.QMainWindow):
    ...
    def find_latest_version_path(self, original_path):
        ...
        resolver_roots = ["I:/", "R:/"]  # From AR PXR DEFAULT RESOLVER
        self.set_log("I:/")
        ...
        if is_relative:
            if self.pathFile:
                rel_based = os.path.normpath(os.path.join(os.path.dirname(self.pathFile), raw_path))
                candidate_paths.append(rel_based)

            # Try mapped AR resolver roots
            for root in resolver_roots:
                candidate_paths.append(os.path.normpath(os.path.join(root, raw_path)))
        else:
            candidate_paths.append(os.path.normpath(raw_path))
        ...
        for resolved_path in candidate_paths:
            ...
            current_version = int(match.group(3)[1:])
            ...
            for fname in os.listdir(latest_folder):
                if re.fullmatch(f"{re.escape(asset_name)}_.+?_{latest_str}\\.usd[ac]", fname, re.IGNORECASE):
                    ...
                    # If original used logical path, preserve the form
                    if raw_path.startswith("intermarche/"):
                        logical_path = raw_path.split("/", 1)[1]
                        updated_path = f"@intermarche/{os.path.relpath(updated_abs_path, resolver_roots[0]).replace('\\', '/')}@"
                    elif is_relative:
                        updated_path = f"@{os.path.relpath(updated_abs_path, os.path.dirname(self.pathFile)).replace('\\', '/')}@"
                    else:
                        updated_path = f"@{updated_abs_path.replace(os.sep, '/')}@"
